...
test('C static fallback models are loaded from registry and contain expected free models', () => {
  ...
  const withStepfun = models.filter((m) => m.modelId === 'stepfun/step-3.7-flash:free');
  expect(
    withStepfun.length >= 1,
    'at least one static fallback model should be stepfun/step-3.7-flash:free'
  );
});
